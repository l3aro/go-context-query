...
GCQ looks for configuration in the following location:

1. `.gcq/config.yaml` - Project-level config (required)

If no config file is found, GCQ will prompt you to run `gcq init` to create one.
